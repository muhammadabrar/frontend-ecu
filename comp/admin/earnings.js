...
export default function Earning(props) {
  ...
  let daycurrentdata = [];
  var today = new Date(6,10,2021)
  var day = today.getDay();
    useEffect(() => {
      const earning = async()=>{
        const req = await axios.get(`${process.env.NEXT_PUBLIC_API}earning/${label}`)
        .then(res => {
   ...
   <h4 className="text-title">Earning </h4>
   <MDBDropdown>
    ...
    <ResponsiveContainer width="100%" height={200} className="earning-chart">
          ...
          >
            ...
            <Area type="monotone" dataKey="_sum.earn" fill="url(#color)"/>
          </AreaChart>
        </ResponsiveContainer>
   </>
      )
